...
async function addCategory(req, res) {
    try {
        ...
        // Vérifier si une catégorie avec le même title ET description existe déjà
        const existingCategory = await Category.findOne({ title: title.trim(), description: description.trim() });
        if (existingCategory) {
            return res.status(409).json({ message: 'Catégorie déjà existante dans la base de données' });
        }
