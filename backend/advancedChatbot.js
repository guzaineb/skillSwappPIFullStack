...
const knowledgeBase = {
  skills: [
    { name: 'JavaScript', category: 'Programmation', description: 'Langage de programmation pour le web' },
    { name: 'Piano', category: 'Musique', description: 'Instrument de musique à clavier' },
    { name: 'Anglais', category: 'Langues', description: 'Langue internationale' },
    // Vous pouvez ajouter plus de compétences ici
  ],
  faqs: [
    { 
      question: 'Comment fonctionne SkillExchange?', 
      answer: 'SkillExchange est une plateforme qui permet aux utilisateurs d\'échanger des compétences gratuitement. Vous pouvez proposer vos compétences et apprendre de nouvelles compétences auprès d\'autres utilisateurs.'
    },
    { 
      question: 'Comment puis-je proposer mes compétences?', 
      answer: 'Vous pouvez proposer vos compétences en créant une offre dans votre profil. Précisez la compétence que vous souhaitez partager, votre niveau d\'expertise et votre disponibilité.'
    },
...
async function detectIntent(message) {
  ...
  try {
      ...
      messages: [
        {
          ...
          content: "Vous êtes un assistant qui analyse l'intention d'un message. Répondez uniquement avec un mot parmi: greeting, farewell, thanks, help, learn, teach, info, other."
        },
        {
          role: "user",
          content: message
    ...
    console.error('Erreur lors de la détection d\'intention avec OpenAI:', error);
    return 'other';
  }
}
...
async function generateResponse(userId, message, intent) {
  ...
  let systemPrompt = "Vous êtes un assistant virtuel pour la plateforme SkillExchange, qui permet aux utilisateurs d'échanger des compétences gratuitement.";
  ...
  switch (intent) {
    ...
    case 'help':
      ...
      break;
    case 'learn':
      systemPrompt += " Donnez des informations sur comment apprendre une compétence sur la plateforme.";
      break;
    case 'teach':
      systemPrompt += " Expliquez comment proposer ses compétences sur la plateforme.";
      break;
    case 'info':
      systemPrompt += " Fournissez des informations générales sur la plateforme.";
      break;
    default:
  ...
  systemPrompt += " Utilisez ces informations si nécessaire: " + JSON.stringify(knowledgeBase);
  ...
  try {
   ...
   content: systemPrompt
    ...
    console.error('Erreur lors de la génération de réponse avec OpenAI:', error);
    return {
